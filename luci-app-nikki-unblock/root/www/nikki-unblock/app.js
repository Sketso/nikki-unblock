...
const I18N = {
  ru: {
    ...
    strat_ytdiscord: "YouTube + Discord + игры (рекомендуется)", strat_default: "Сбалансированная", strat_youtube: "YouTube: QUIC-блоб googlevideo",
    ...
    z2QuicHint: "Включите, если сайт настроен верно, но в браузере всё равно не открывается или открывается через раз.",
    ...
    taEscFine: n => "Остальные потоки шли мимо туннеля и работали — им отвечали. Всего: " + n,
    ...
    apLabel: "Перехватывать все порты (TCP и UDP)", apHint: "Без него часть трафика уходит мимо VPN — так ломаются медиа в Telegram и звонки. Лучше держать включённым. Переключение перезапускает nikki: сеть моргнёт на несколько секунд.", mssLabel: "Фикс зависаний загрузок (MSS-clamp)", mssHint: "Включи, если через VPN большие загрузки или сайты зависают / грузятся наполовину, а мелочь при этом работает. Чинит размер сетевых пакетов под туннель. Оставлять включённым безопасно.",
  ...
  en: {
    ...
    strat_ytdiscord: "YouTube + Discord + games (recommended)", strat_default: "Balanced", strat_youtube: "YouTube: googlevideo QUIC blob",
    ...
    z2QuicHint: "Enable if a site is set up correctly but still won't open in the browser, or opens only sometimes.",
    ...
    taEscFine: n => "The remaining flows went past the tunnel and worked — they were answered. Total: " + n,
