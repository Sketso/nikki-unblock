...
const I18N = {
  ru: {
    ...
    z2QuicHint: "HTTP/3 идёт мимо обоих движков, поэтому сайт бывает настроен верно и всё равно не работает. Блокировка возвращает браузеры на TCP, где обход действует; потеря скорости незаметна. Приложению YouTube на телефоне не поможет.",
  ...
  en: {
    ...
    z2QuicHint: "HTTP/3 slips past both engines, so a site can be set up correctly and still fail. Blocking it returns browsers to TCP, where the bypass works; the speed cost is unnoticeable. Won't help the mobile YouTube app.",
